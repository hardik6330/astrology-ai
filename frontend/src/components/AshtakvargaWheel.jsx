    ...
    <div className="cosmic-card">
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
          {ashtakvarga.perSign.map((s, i) => {
            ...
            return (
              <g key={s.sign}>
                <path d={sector(a0, a1, rOut, R_INNER)} fill={fillFor(s.total)} opacity={0.75}
                      stroke="#050508" strokeWidth={1} />
                <text x={mid.x} y={mid.y + 3} fontSize="9" fill="#cbd5e1" textAnchor="middle">
                  {s.sign.slice(0, 3)}
                </text>
                <text x={inner.x} y={inner.y + 3} fontSize="11" fontWeight="700" fill="#fff" textAnchor="middle">
                  {s.total}
                </text>
              </g>
            );
          ...
          <text x={CX} y={CY - 4}  fontSize="10" fill="#64748b" textAnchor="middle">SARVA</text>
          <text x={CX} y={CY + 12} fontSize="14" fontWeight="700" fill="#fff" textAnchor="middle">{total}/337</text>
        </svg>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 10, marginTop: 12 }}>
        {[
          ...
          <span key={label} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10.5, color: "#cbd5e1" }}>
