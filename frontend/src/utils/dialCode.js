// Maps the browser's locale region to an E.164 dialing code, so the web login
// can pre-fill the right country code with no permission prompt, network call,
// or GPS. Region comes from the browser/OS locale (Intl + navigator.language) —
// it tracks the user's configured country far better than IP geolocation (an
// Indian user abroad still has region=IN, so they get +91, not the local code).
//
// The detected code is only a DEFAULT — the login prefix stays editable, so a
// wrong/unknown region is a quick manual fix, never a lockout. Twin of the
// mobile util (mobile/src/utils/dialCode.js) — keep the ISO→dial map in sync.
...
// The browser's region as an ISO code, e.g. "IN" / "US". Null if unavailable.
// Prefers Intl's resolved locale (carries the region even when the language has
// none, e.g. "en-US"), then falls back to navigator.language.
function browserRegion() {
  try {
    ...
    // Intl.Locale unsupported / malformed tag — fall through to null.
  }
  return null;
}

// Best-guess dialing code (digits, no "+") for the browser's region, or "" when
