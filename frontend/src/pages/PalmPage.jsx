...
export default function PalmPage() {
  ...
  async function runAnalyze(dataUrl) {
    ...
    const iv = setInterval(() => { i++; setScanMsg(SCAN_MSGS[i % SCAN_MSGS.length]); }, 1800);
    try {
      const result = await analyzePalm(dataUrl, form);
      setPalm(result);
      setRescan(false);
    } catch (err) {
      if (err.code === 'AI_OVERLOADED') {
        setOverloaded(true);
        setCooldown(40);   // disable retry button for 40s
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1rem 7.5rem", position: "relative" }}>
          ...
          {!preview && !scanning && (
            <>
              ...
              <p style={{ fontSize: 12, color: "#94a3b8", margin: "0 0 20px", lineHeight: 1.6 }}>
                Take a clear photo of your dominant hand. Open your palm flat, good lighting,
                fingers slightly spread so the four major lines are visible.
              </p>
              {hasCamera ? (
                <div style={{ display: "grid", gap: 10 }}>
                  <button onClick={() => cameraRef.current?.click()} className="magic-btn" style={{ width: "100%" }}>
                    📷 Take Photo
                  </button>
                  <button onClick={() => fileRef.current?.click()} style={{
                    width: "100%", padding: "12px", borderRadius: 10, fontSize: 13.5, fontWeight: 600,
                ...
                <button onClick={() => fileRef.current?.click()} className="magic-btn" style={{ width: "100%" }}>
                  🖼️ Upload Palm Photo
                </button>
              )}
              <input
      ...
      {palm && !unusable && (
        <div style={{ animation: "slideUp 0.8s ease-out" }}>
          ...
          {preview && (
            <div className="cosmic-card" style={{ padding: 14 }}>
