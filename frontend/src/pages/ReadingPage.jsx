...
import { signOf, ZE, fmtDate, fmtDay, computeDaily, CITIES, buildFactSheet } from "../astrology";
...
export default function ReadingPage() {
  ...
  const dailyTransit = useMemo(
    () => computeDaily(chart, cityObj, selDate),
  ...
  async function generateReading() {
    ...
    try {
      ...
      setInterp(saved || await chatCompletionJSON([], 'interpret', { factSheet: buildFactSheet(chart, form), form }));
    } catch (e) {
...
{/* Common header — Big Three + Nakshatra + Daily Insights. Shown ONLY on
        ...
        <div className="cosmic-card">
          ...
          <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 8, paddingBottom: 8, scrollbarWidth: "thin", scrollbarColor: "rgba(168,85,247,0.5) transparent" }}>
            {monthDays.map((d) => {
              const dIso = iso(d);
              ...
              return (
                <button key={dIso} ref={isToday ? todayBtnRef : null} onClick={() => selectDay(d)} style={{
                  flex: "0 0 auto", width: 50, padding: "7px 0", borderRadius: 12, cursor: "pointer",
                  position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
                  border: "1px solid " + (active ? "rgba(168,85,247,0.7)" : hasData ? "rgba(74,222,128,0.4)" : "rgba(255,255,255,0.08)"),
                  background: active ? "rgba(168,85,247,0.22)" : hasData ? "rgba(74,222,128,0.08)" : "rgba(255,255,255,0.03)",
                  color: active ? "#c084fc" : hasData ? "#86c8a0" : "#94a3b8",
                }}>
                  <span style={{ fontSize: 9, letterSpacing: "0.5px" }}>{isToday ? "TODAY" : WD_SHORT[d.getDay()]}</span>
                  <span style={{ fontSize: 16, fontWeight: 700 }}>{d.getDate()}</span>
                  <span style={{ fontSize: 8, opacity: 0.7 }}>{MONTHS[d.getMonth()]}</span>
                  {hasData && <span style={{ position: "absolute", top: 4, right: 6, width: 5, height: 5, borderRadius: "50%", background: "#4ade80" }} />}
                </button>
              );
            })}
          </div>

          <p style={{ fontSize: 26, fontWeight: 800, color: "#c084fc", margin: "0 0 2px" }}>{dailyTransit.alignment}% Aligned</p>
          {guide && <p style={{ fontSize: 15, fontWeight: 700, color: "#fff", margin: "6px 0 4px" }}>{guide.dayTitle}</p>}
          {guide && <p style={{ fontSize: 13, color: "#cbd5e1", lineHeight: 1.65, margin: "0 0 12px" }}>{guide.intro}</p>}
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
            {[["WEAR", dailyTransit.luckyColor], ["MANIFEST", "◆ " + dailyTransit.luckyNumber],
          ...
          {guide && guide.action && (
            <div style={{ borderLeft: "2px solid #c084fc", paddingLeft: 12, margin: "0 0 14px" }}>
              ...
              <p style={{ fontSize: 13.5, color: "#e2e8f0", fontStyle: "italic", margin: "3px 0 0" }}>"{guide.action}"</p>
            </div>
          )}

          {!guide && (
            <button onClick={() => loadDaily(selDate)} disabled={dailyBusy} style={{
          ...
          {guide && (
            <div style={{ display: "grid", gap: 10 }}>
              {[["SELF", guide.self], ["LOVE", guide.love], ["RELATIONSHIP", guide.relationship],
                ...
                ["WEALTH", guide.wealth], ["SPIRITUAL", guide.spiritual], ["AVOID", guide.avoid]].map(([l, v]) => v && (
                <div key={l}>
        ...
        {overloaded && !interp && (
          <div className="cosmic-card" style={{ textAlign: "center", borderColor: "rgba(251,191,36,0.4)", background: "rgba(251,191,36,0.08)" }}>
            ...
            <button onClick={generateReading} disabled={cooldown > 0} className="magic-btn" style={{ width: "100%", opacity: cooldown > 0 ? 0.5 : 1, cursor: cooldown > 0 ? "not-allowed" : "pointer" }}>
        ...
        {interp && (
          <div style={{ animation: "slideUp 0.8s ease-out" }}>
            <div style={{
              ...
              <p style={{ fontSize: "clamp(15px, 4.2vw, 20px)", fontWeight: 600, color: "#fff", margin: 0, lineHeight: 1.6, fontStyle: "italic" }}>"{asText(interp.lifeTheme)}"</p>
            </div>
              ...
              {[
                ...
                <div key={title} className="cosmic-card" style={{ margin: 0, animationDelay: `${idx * 0.1}s` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                    <span className="astrology-icon" style={{ margin: 0, fontSize: 22 }}>{icon}</span>
            ...
            <div className="grid-2" style={{ margin: "20px 0" }}>
              <div className="cosmic-card" style={{ margin: 0, borderColor: "rgba(34, 197, 94, 0.2)", background: "rgba(20, 30, 20, 0.4)" }}>
                ...
                {(interp.strengths || []).map((s, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 14, color: "#94a3b8", lineHeight: 1.5 }}>
                    <span style={{ color: "#4ade80", fontWeight: 700 }}>✓</span><span>{asText(s)}</span>
                  </div>
                ))}
              </div>
              <div className="cosmic-card" style={{ margin: 0, borderColor: "rgba(251, 191, 36, 0.2)", background: "rgba(30, 25, 20, 0.4)" }}>
                ...
                {(interp.challenges || []).map((c, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 14, color: "#94a3b8", lineHeight: 1.5 }}>
                    <span style={{ color: "#fbbf24", fontWeight: 700 }}>↑</span><span>{asText(c)}</span>
                  </div>
                ))}
              </div>
            </div>
