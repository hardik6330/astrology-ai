...
- Chart math (single source): `packages/astrology-core/src/astrology.js` (`@astrology-ai/core`, incl. `transits.gochar` live-sky array); the clients are thin proxies `frontend/src/shared/astrology.js` + `mobile/src/shared/astrology.js` — edit core, not the proxies
