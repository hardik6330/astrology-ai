// Static Vedic-planet reference for the interactive PlanetDetail sheet. Keyed by
// the planet's BASE name (chart planets store `p.base`, e.g. "Rahu" from
// "Rahu (N.Node)"). Content is fixed astrological meaning — the placement line
// (sign/house) is filled in dynamically by the sheet from the live chart, so
// nothing here is invented per-user.
...
export const DASHA_GUIDANCE = {
  Sun: {
    dos: ["Step into leadership roles", "Build ties with mentors and authority", "Prioritize health and vitality"],
    donts: ["Let ego drive decisions", "Clash with bosses or your father", "Overwork to the point of burnout"],
  },
  Moon: {
  ...
  Rahu: {
    dos: ["Pursue bold, unconventional goals", "Embrace tech, foreign or new fields", "Stay grounded amid ambition"],
...
export function dashaGuidanceFor(period) {
  ...
  const parts = String(period).split("–").map((x) => x.trim());
