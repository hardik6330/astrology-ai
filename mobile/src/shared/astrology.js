...
export const ZE = {Aries:"♈",Taurus:"♉",Gemini:"♊",Cancer:"♋",Leo:"♌",Virgo:"♍",Libra:"♎",Scorpio:"♏",Sagittarius:"♐",Capricorn:"♑",Aquarius:"♒",Pisces:"♓"};
...
export function fmtDate(d){return d.toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}
export function fmtDay(d){return d.toLocaleDateString("en-GB",{day:"2-digit",month:"short"})}
...
export function signOf(lon){return SIGNS[Math.floor(nm(lon)/30)]}
...
export function computeChart(dateStr,timeStr,city){
  ...
  if(P.Jupiter&&P.Moon){
    ...
    if([1,4,7,10].includes(dd)) yogas.push("Gajakesari Yoga — Jupiter in a quadrant from the Moon (wisdom, fortune)");
  }
  ...
  try{
    ...
    const collectEcl=(searchFn,nextFn,type,axisOffset)=>{
      ...
      for(let i=0;i<2&&ev&&ev.peak;i++){
        ...
        let hit=null;
        planets.forEach(p=>{const dd=Math.abs(nm(p.sid)-elon);if(Math.min(dd,360-dd)<5)hit=p.name;});
  ...
  tBodies.forEach(tb=>{
    planets.slice(0,10).forEach(np=>{
      ...
      ASPECTS.forEach(([an,ang])=>{if(Math.abs(sep-ang)<3)chart.transits.tnAspects.push({t:tb.n,n:np.name,type:an,orb:Math.abs(sep-ang).toFixed(1)})});
    });
  });
...
function computeDoshas(ch){
  ...
  if(rahu && ketu){
    const rL=nm(rahu.sid), kL=nm(ketu.sid);
    const grahas=["Sun","Moon","Mars","Mercury","Jupiter","Venus","Saturn"]
      .map(b=>P[b]).filter(Boolean);
    ...
    if(allInRahuKetu || allInKetuRahu){
      ...
      kaalSarp={present:false,detail:"No nodal alignment — your planets fall on both sides of the North–South Node axis."};
    }
  }
  ...
  if(sun && rahu){
    ...
    if(sunRahuConj){
      pitra={present:true,detail:`Sun–North Node conjunction in House ${sun.houseSid} — ancestral karma activation.`};
    } else if(malefIn9){
      ...
      pitra={present:true,detail:`${whichEn} in the 9th house — paternal/ancestral themes need attention.`};
    } else {
...
const KARANA_NAMES=["Bava","Balava","Kaulava","Taitila","Garaja","Vanija","Vishti","Shakuni","Chatushpada","Naga","Kintughna"];
const YOGA_NAMES=["Vishkambha","Preeti","Ayushmana","Saubhagya","Shobhana","Atiganda","Sukarmana","Dhriti","Shoola","Ganda","Vriddhi","Dhruva","Vyaghata","Harshana","Vajra","Siddhi","Vyatipata","Variyana","Parigha","Shiva","Siddha","Sadhya","Shubha","Shukla","Brahma","Indra","Vaidhriti"];

function computePanchang(ch, birthDate){
  const sun=ch.planets[0], moon=ch.planets[1];
  ...
  const tithiName = tithiNum===15
    ? (paksha==="Shukla" ? "Purnima" : "Amavasya")
    : TITHI_NAMES[tithiNum-1];
  ...
  else if(karanaIdx>=57)          karana=KARANA_NAMES[7+(karanaIdx-57)]; // Shakuni..Kintughna
...
function computeStrengthMeter(ch){
  ...
  const find=b=>ch.planets.find(p=>p.base===b);
  return GRAHAS.map(b=>{
    const p=find(b); if(!p) return {planet:b,score:0,label:"—"};
    let s=50;
    let label="Neutral";
    if(/exalt/.test(p.dignity||""))      { s=92; label="Exalted"; }
    else if(/in own/.test(p.dignity||"")) { s=80; label="Own Sign"; }
    else if(/debil/.test(p.dignity||""))  { s=15; label="Debilitated"; }
    if(p.angular) s+=10;
...
const ASHTAK_RULES = {
  Sun:     { Sun:[1,2,4,7,8,9,10,11], Moon:[3,6,10,11], Mars:[1,2,4,7,8,10,11], Mercury:[3,5,6,9,10,11,12], Jupiter:[5,6,9,11], Venus:[6,7,12], Saturn:[1,2,4,7,8,9,10,11], Asc:[3,4,6,10,11,12] },
  ...
  Venus:   { Sun:[8,11,12], Moon:[1,2,3,4,5,8,9,11,12], Mars:[3,4,6,9,11,12], Mercury:[3,5,6,9,11], Jupiter:[5,8,9,10,11], Venus:[1,2,3,4,5,8,9,10,11], Saturn:[3,4,5,8,9,10,11], Asc:[1,2,3,4,5,8,9,11] },
...
function computeAshtakvarga(ch){
  ...
  const signOfBody=b=>{
    ...
    const p=find(b); return p?Math.floor(nm(p.sid)/30):null;
  };
  ...
  Object.values(bhinna).forEach(arr=>arr.forEach((v,i)=>{ sarva[i]+=v; }));
